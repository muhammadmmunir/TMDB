...
class APIServiceTests: XCTestCase {
    ...
    func test_whenMalformedUrl_shouldReturnUrlGenerationError() {
        // given
        let config = MockAPIServiceConfig()
        let expectation = self.expectation(description: "Should return generation error")
        ...
        // when
        _ = self.sut.request(
            endpoint: MockAPIEndpoint(path: "$%***!", method: .get)
        ) { result in
            do {
                _ = try result.get()
                XCTFail()
            } catch let error {
                if case APIServiceError.urlGeneration = error {
                    expectation.fulfill()
                } else {
                    XCTFail()
                }
